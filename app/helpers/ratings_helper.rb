module RatingsHelper
  def rating(product, opts)
    ...
    if opts[:count]
      r = product.rates(dimension).count
      cnt = "(#{r})" if r > 0
      h[:readonly] = true
    end

    content_tag :div, class: 'rating_container' do
      concat(rating_for product, dimension, h)
      concat(content_tag(:span, cnt))
  ...
  def rating_for(rateable_obj, dimension=nil, options={})
    ...
    avg = options[:init_mode] ? 0 : avg
    if options[:user]
      @rating = Rate.find_by_rater_id_and_rateable_id_and_dimension(options[:user].id, rateable_obj.id, dimension)
      avg = @rating ? @rating.stars : 0
    end
